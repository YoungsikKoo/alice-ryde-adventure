...
_cheatLoad(sub){
  ...
  this.entities=this.entities.filter(e=>e instanceof Player);
  s._loadSub(this,sub);
  this._notifyStageChange("s1_"+sub);
}
_cheatLoadStage2(floor){
  ...
  this._notifyStageChange("s2_"+(floor||"b2"));
}
_cheatLoadStage3(){
  if(this.sound)this.sound.stopBGM();
  this.tileMap=null;
  var p=this.localPlayer;
  p.sideScrollMode=false;p.vx=0;p.vy=0;
  p.hp=p.maxHp;
  this.entities=this.entities.filter(e=>e instanceof Player);
  this.state="playing";
